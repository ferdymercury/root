                        ...
                        cling::Interpreter &interp)
{
   ...
   if (method != nullptr && (method->getAccess() == clang::AS_public || method->getAccess() == clang::AS_none)) {
      ...
      if (strstr(filename.c_str(), "TBuffer.h") != nullptr ||
            strstr(filename.c_str(), "Rtypes.h") != nullptr) {
         ...
         if (strncmp(comment, "!", 1)) {
            ...
            if (underling_type->isFundamentalType() || underling_type->isEnumeralType()) {
               if (type.getTypePtr()->isConstantArrayType() &&
                  ...
                  if (i == 0) {
                     ROOT::TMetaUtils::Error(nullptr, "*** Datamember %s::%s: array of pointers to fundamental type (need manual intervention)\n", fullname.c_str(), field_iter->getName().str().c_str());
               ...
               if (type.getTypePtr()->isConstantArrayType() &&
                  ...
                  if (isPointerToPointer(**field_iter)) {
                     if (i == 0) {
                        ROOT::TMetaUtils::Error(nullptr, "*** Datamember %s::%s: pointer to pointer (need manual intervention)\n", fullname.c_str(), field_iter->getName().str().c_str());
                        dictStream << "      //R__b.ReadArray(" << field_iter->getName().str() << ");" << std::endl;
                     } else {
                        ...
                        std::string &rootmapLibName)
{
   ...
   if (rootmapFileName.empty()) {
      ...
      ROOT::TMetaUtils::Info(nullptr, "Rootmap file name %s built from rootmap lib name %s",
                     ...
                     bool isSelXML, bool writeEmptyRootPCM)
{
   ...
   for (auto const &selClass : scan.fSelectedClasses) {
      ...
      ROOT::TMetaUtils::WriteClassCode(&CallWriteStreamer, selClass, interp, normCtxt, dictStream, ctorTypes,
                                       isGenreflex);
   }
...
// TODO: Maybe move these global variables to a RootClingConfig object
// (keeping this as globals minimizes the diff after llvm::cl removal)
...
public:
   ...
   void HandleDiagnostic(clang::DiagnosticsEngine::Level DiagLevel, const clang::Diagnostic &Info) override
   {
      ...
      if (ID == remark_module_build || ID == remark_module_build_done) {
         ...
         if (!module) {
            ROOT::TMetaUtils::Warning(nullptr,
...
// FIXME: Copied and adapted from previous help menu, would be nice if optparse supports generating this automatically.
static const char *const kShortHelp = "USAGE: rootcling [subcommand] [options] <output dictionary file> <list of "
                                      "dictionary header files> <LinkDef file | selection xml file>\n";
...
rootcling common options:
  ...
  -U <string>              - Specify undefined macros.
  ...
  --reflex                 - Behave internally like genreflex.
   ...
   if (!ModuleContainsHeaders(modGen, headerSearch, module, missingHdrMod)) {
      ...
      if (!gOptUmbrellaInput && maybeUmbrella) {
         ROOT::TMetaUtils::Info("CheckModuleValid, %s. You can silence this message by adding %s to the invocation.",
                                warningMessage.c_str(), "umbrellaHeader");
         return true;
      }
              ...
              bool isGenreflex = false)
{
   ...
   // Check if first positional arg is the subcommand, we pass everything directly to cling.
   // FIXME: It would be nice to have optparse handle subcommands, but this works for now.
   const char *etcDir = gDriverConfig->fTROOT__GetEtcDir();
   std::string llvmResourceDir = etcDir ? std::string(etcDir) + "/cling" : "";

   if (argc > 0 && std::string(argv[1]) == "bare-cling") {
      ...
      // Push all args after 'bare-cling' exactly as they were passed
      for (int i = 2; i < argc; ++i) {
         llvm::errs() << argv[i] << "\n";
         clingArgsC.push_back(argv[i]);
      }
   ...
   // FIXME: if -v2 and -v4 is set, should we set -v4 by default?
   // From CommandLine docs:
   // Options that are specified multiple times are redundant. Any instances after the first are discarded.
   if (opts.GetSwitch("v0"))
      gOptVerboseLevel = v0;
   if (opts.GetSwitch("v1") || opts.GetSwitch("v"))
      gOptVerboseLevel = v;
   if (opts.GetSwitch("v2"))
      gOptVerboseLevel = v2;
   if (opts.GetSwitch("v3"))
      gOptVerboseLevel = v3;
   if (opts.GetSwitch("v4"))
      gOptVerboseLevel = v4;
   ...
   // Positional args: rootcling, <output dictionary file>, <list of dictionary header files>
   auto positionalArgs = opts.GetArgs();
   if (!positionalArgs.empty()) {
      gOptDictionaryFileName = positionalArgs[1];
   } else {
      ROOT::TMetaUtils::Error(nullptr, "No dictionary file specified!\n");
      return 1;
   }

   for (size_t i = 2, e = positionalArgs.size(); i < e; ++i) {
      gOptDictionaryHeaderFiles.push_back(positionalArgs[i]);
   }
   ...
   if (!gOptISysRoot.empty()) {
      ...
      clingArgs.push_back(gOptISysRoot);
   }
   ...
   bool isPCH = (gOptDictionaryFileName == "allDict.cxx");
   ...
   if (gOptSharedLibFileName.empty()) {
      gOptSharedLibFileName = gOptDictionaryFileName;
   }
...
#include "clang/Basic/LangOptions.def"
   ...
   for (size_t i = 0, e = gOptDictionaryHeaderFiles.size(); i < e; ++i) {
      ...
      std::string header(
         isSelectionFile ? fullheader : ROOT::FoundationUtils::MakePathRelative(fullheader, currentDirectory, gBuildingROOT));
   ...
   // Add the diagnostic pragmas distilled from the -Wno-xyz
   {
      ...
      if (interp.declare(res.str()) != cling::Interpreter::kSuccess) {
         ROOT::TMetaUtils::Error(nullptr, "Failed to parse -Wno-xyz flags as pragmas:\n%s", res.str().c_str());
         return 1;
      }
   }
   ...
   if (!gDriverConfig->fBuildingROOTStage1) {
      ...
      if (!definesUndefinesStr.str().empty()) {
         if (interp.declare(definesUndefinesStr.str()) != cling::Interpreter::kSuccess) {
            ROOT::TMetaUtils::Error(nullptr, "Failed to parse -D, -U flags as preprocessor directives:\n%s", definesUndefinesStr.str().c_str());
            return 1;
         }
      }
   }
   ...
   string main_dictname(gOptDictionaryFileName);
   ...
   if (!gOptDictionaryFileName.empty()) {
      tmpCatalog.addFileName(gOptDictionaryFileName);
   ...
   std::ostream &dictStream = (!gOptDictionaryFileName.empty()) ? fileout : std::cout;
   bool isACLiC = gOptDictionaryFileName.find("_ACLiC_dict") != std::string::npos;

   // Now generate a second stream for the split dictionary if it is necessary
   if (gOptSplit) {
      splitDictStream = CreateStreamPtrForSplitDict(gOptDictionaryFileName, tmpCatalog);
...
int GenReflexMain(int argc, char **argv)
{
   ...
   if (options[DEEP])
      ROOT::TMetaUtils::Warning(nullptr, "--deep has no effect. Please remove the deprecated flag!\n");
